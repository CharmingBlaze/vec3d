...
export function exportSVG() {
  ...
  const svgOut = `<svg xmlns="http://www.w3.org/2000/svg" width="${state.canvasW}" height="${state.canvasH}">${dom.shapesLayer.innerHTML}</svg>`;
...
export function initPathOps() {
  ...
  dom.opBool.onclick = () => {
    if (ctx.state.selected.length < 2) {
      alert('Select at least 2 paths to combine.');
      return;
    }
    const paths = ctx.state.selected.map((id) => getObj(id)).filter((o) => o?.type === 'path');
    if (paths.length < 2) {
      alert('Boolean union works on path objects. Flatten shapes first.');
      return;
    }
    const ds = paths.map((o) => o.el.getAttribute('d')).filter(Boolean).join(' ');
    getScene().removeMany(paths.map((o) => o.id));
    const el = svgEl('path', {
      d: ds,
      'fill-rule': 'evenodd',
      fill: paths[0].fill,
      stroke: paths[0].stroke,
      'stroke-width': paths[0].sw,
      opacity: paths[0].op,
    });
